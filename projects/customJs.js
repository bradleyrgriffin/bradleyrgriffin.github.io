...
var MUTATE_PERCENTAGE = 0.5; //Better off to leave this as is, changing it even a small amount drastically affects the algorithm
var MIN = 2;
var MAX = 20;
...
var numAttempts = 0;
var csvForExcelAllData =
  'Generation' +
  ',' +
  'Generation Fitness Level' +
  ',' +
  'Generation Total Volume' +
  ',' +
  'Number of Cubes' +
  ',' +
  'Number of Pyramids' +
  ',' +
  'Number of Shapes in Knapsack' +
  ',' +
  'Volume Wasted in Knapsack' +
  '\n';
...
function main() {
  ...
  for (var g = 0; g < MAX_NUM_GENERATIONS; g++) {
    ...
    for (var z = 0; z < nextGenPop.length; z++) {
      if (nextGenPop[z].volume + nextGenPop[z].wastedSpace < knapsackFilled) {
        knapsackFilled =
          knapsackFilled - (nextGenPop[z].volume + nextGenPop[z].wastedSpace);
        numInSack++;
      }
    }
    ...
    bdyText +=
      ...
      g +
      '</b></h4>' +
      '<ul><li>Fitness Score of Generation: ' +
      sumGenFitness +
      '</li><li>Total Volume of Generation: ' +
      sumtotVol +
      '</li><li>Cubes in Generation: ' +
      numCubes +
      '</li><li>Pyramids in Generation: ' +
      numPyramids +
      '</li><li>Objects in Knapsack: ' +
      numInSack +
      '</li><li>Volume left in Knapsack: ' +
      knapsackFilled +
      '</li></ul></div>';
  ...
  lastGenFitnessForChart.push([
    "'" + numAttempts + "'",
    chartArray[chartArray.length - 1][1],
  ]);

  //Appends Total Number of Generations
  $('#num-clicks').html(
    '<h4>Total Number of Generations: <b><u>' + TotalClicks + '</u></b><h4>'
  );

  //Appends ordered list of data
  $('#content-body').html(
    '<div id="' + getRandomInt(1000, 10000) + '">' + appendAllAtOnce + '</div>'
  );
  ...
  for (var i = 0; i < nextGenPopBeforeMutate.length; i++) {
    if (getRandomArbitrary(0, 1) < MUTATE_PERCENTAGE) {
      ...
      if (nextGenPopBeforeMutate[i].type == 'Cube') {
        nextGenPopBeforeMutate[i].volume =
          nextGenPopBeforeMutate[i].length *
          nextGenPopBeforeMutate[i].width *
          nextGenPopBeforeMutate[i].height;
      } else {
        nextGenPopBeforeMutate[i].volume = Math.floor(
          (nextGenPopBeforeMutate[i].length *
            nextGenPopBeforeMutate[i].width *
            nextGenPopBeforeMutate[i].height) /
            3
...
function chooseRandomParent() {
  ...
  if (workingVal < MAX_NUM_POPULATION_MEMBERS * 8)
    // First quarter
    return Math.floor(workingVal % (MAX_NUM_POPULATION_MEMBERS / 4));
  else if (workingVal < MAX_NUM_POPULATION_MEMBERS * 12)
    // Second quarter
    return Math.floor(
      (workingVal % (MAX_NUM_POPULATION_MEMBERS / 4)) +
        MAX_NUM_POPULATION_MEMBERS / 4
    );
  else if (workingVal < MAX_NUM_POPULATION_MEMBERS * 14)
    // Third quarter
    return Math.floor(
      (workingVal % (MAX_NUM_POPULATION_MEMBERS / 4)) +
        (MAX_NUM_POPULATION_MEMBERS / 4) * 2
    );
  // Last quarter
  else
    return Math.floor(
      (workingVal % (MAX_NUM_POPULATION_MEMBERS / 4)) +
        (MAX_NUM_POPULATION_MEMBERS / 4) * 3
    );
}
...
function createChildren(initPop) {
  ...
  for (var i = 0; i < initPop.length / 2; i++) {
    ...
    while (p1 == p2) {
      p2 = chooseRandomParent();
    } //Ensures two different parents.

    var firstChild = {
      width: initPop[p1].width,
    ...
    switch (firstChild.type) {
      case 'Cube':
        firstChild.volume =
          firstChild.width * firstChild.height * firstChild.length;
        break;
      case 'Pyramid':
        firstChild.volume = Math.floor(
          (firstChild.width * firstChild.height * firstChild.length) / 3
        );
        break;
    }
    ...
    switch (secondChild.type) {
      case 'Cube':
        secondChild.volume =
          secondChild.width * secondChild.height * secondChild.length;
        break;
      case 'Pyramid':
        secondChild.volume = Math.floor(
          (secondChild.width * secondChild.height * secondChild.length) / 3
        );
        break;
    }
...
function createNewPopulationMember(curPopulation) {
  ...
  switch (getRandomInt(0, 1)) {
    ...
    case 1:
      ...
      newMember.volume = Math.floor(
        (newMember.length * newMember.width * newMember.height) / 3
      );
      break;
      ...
      if (curPopulation.length > 0) {
        for (var i = 0; i < curPopulation.length; i++) {
          if (
            curPopulation[i].width == newMember.width &&
            curPopulation[i].height == newMember.height &&
            curPopulation[i].length == newMember.length &&
            newMember.type == curPopulation[i].type
          ) {
            isUnique = false;
          }
        }
